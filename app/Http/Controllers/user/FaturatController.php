...
/*    public function search(Request $request){
    ...
    public function shikoFaturatData($id)
    {
        ...
        $faturat = Faturat::leftJoinSub(function ($query) {
            $query->select('id_fatures',
                \DB::raw('SUM(cmimi_total) as total_cmimi'),
                \DB::raw('SUM(blerjet_total) as total_blerja_total'))
                ->from('faturat_detajet')
                ->groupBy('id_fatures');
        }, 'q1', function ($join) {
            $join->on('faturat.nr_fature', '=', 'q1.id_fatures');
        })
            ->whereDate('faturat.date', $date)
            ->where('faturat.agjent_id', $agjenti)
            ->latest()
            ->get();
    ...
    public function prodPerTuFshire($id)
    {
        DB::transaction(function () use ($id) {
            ...
            $faturatDetajet = FaturatDetajet::findOrFail($id);

            // Retrieve related product and update quantity
            $produkti = Produkti::find($faturatDetajet->produkt_id);
            if ($produkti) {
                $produkti->increment('quantity', $faturatDetajet->sasia_shitur);
            }

            // Delete invoice detail
            $faturatDetajet->delete();
        });
