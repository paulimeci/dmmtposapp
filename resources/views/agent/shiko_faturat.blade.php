                            ...
                            <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                @foreach($faturat->sortByDesc('nr_fature') as $key => $item)
                                    <tr>
                                        ...
                                        <td>{{ \Carbon\Carbon::parse($item->date)->translatedFormat('d F') }}</td>
                                        <td>{{ $item->total_cmimi }}</td>
                                        <td>{{ $item->zbritjet->shifra ?? 0 }} %</td>
                                        <td>{{ number_format($item->total_cmimi - ((is_numeric($item->zbritjet) ? $item->zbritjet : 0)) - $item->total_blerja_total, 2) }}</td>
